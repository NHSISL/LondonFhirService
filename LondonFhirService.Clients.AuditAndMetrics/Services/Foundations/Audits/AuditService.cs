...
namespace LondonFhirService.Clients.AuditAndMetrics.Services.Foundations.Audits
{
    internal partial class AuditService : IAuditService
    {
        ...
        SwallowAsync(() => TryCatch(async () =>
        {
            ...
            Dispatch(async token => await this.auditBroker.InsertAuditAsync(audit, token));
        }));
        ...
        TryCatch(async () =>
        {
            ...
            return await this.auditBroker.InsertAuditAsync(audit, cancellationToken);
        });
        ...
        SwallowAsync(() => TryCatch(async () =>
        {
            ...
            for (int index = 0; index < audits.Count; index += batchSize)
            {
                ...
                Dispatch(token => this.auditBroker.BulkInsertAuditsAsync(batch, token));
            }
        }));
        ...
        TryCatch(async () =>
        {
            ...
            for (int index = 0; index < audits.Count; index += batchSize)
            {
                ...
                List<IAudit> batch = audits.Skip(index).Take(batchSize).ToList();
                await this.auditBroker.BulkInsertAuditsAsync(batch, cancellationToken);
            }
        });
        ...
        TryCatch(async () =>
        {
            ...
            return await this.auditBroker.SelectAllAuditsAsync(cancellationToken);
        });
        ...
        TryCatch(async () =>
        {
            ...
            IAudit maybeAudit = await this.auditBroker.SelectAuditByIdAsync(auditId, cancellationToken);
        ...
        TryCatch(async () =>
        {
            ...
            IAudit maybeAudit = await this.auditBroker.SelectAuditByIdAsync(audit.Id, cancellationToken);
            ValidateStorageAudit(maybeAudit, audit.Id);
            ValidateAgainstStorageAuditOnModify(inputAudit: audit, storageAudit: maybeAudit);

            return await this.auditBroker.UpdateAuditAsync(audit, cancellationToken);
        });
        ...
        TryCatch(async () =>
        {
            ...
            return await this.auditBroker.DeleteAuditAsync(maybeAudit, cancellationToken);
        });
            ...
            string logLevel)
        {
            ...
            IAudit audit = this.auditBroker.CreateAudit();
