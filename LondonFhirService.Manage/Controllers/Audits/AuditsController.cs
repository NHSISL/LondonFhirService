...
namespace LondonFhirService.Manage.Controllers.Audits
{
    /// <summary>
    /// Audit rows carry whole patient payloads in Message, so this lives on the internal
    /// management host rather than the public API - Manage is reachable only from the business IP
    /// range.
    ///
    /// Reads are open to the same roles as the rest of this host. Create, update and delete carry
