...
namespace LondonFhirService.Manage.Tests.Unit.Controllers.Providers
{
    public partial class ProvidersControllerTests
    {
        ...
        public async Task ShouldReturnRecordOnGetByIdsAsync()
        {
            ...
            providerServiceMock
                .Setup(service => service.RetrieveProviderByIdAsync(
                    It.IsAny<Guid>(),
                    It.IsAny<CancellationToken>()))
                        .ReturnsAsync(storageProvider);

            // when
            ActionResult<Provider> actualActionResult = await providersController.GetProviderByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            providerServiceMock
                .Verify(service => service.RetrieveProviderByIdAsync(
                    It.IsAny<Guid>(),
                    It.IsAny<CancellationToken>()),
                        Times.Once);
