    ...
    public partial class FhirRecordDifferencesControllerTests
    {
        ...
        public async Task ShouldReturnOkOnPutAsync()
        {
            ...
            fhirRecordDifferenceServiceMock
                .Setup(service => service.ModifyFhirRecordDifferenceAsync(inputFhirRecordDifference))
                    .ReturnsAsync(storageFhirRecordDifference);

            // when
            ActionResult<FhirRecordDifference> actualActionResult = await fhirRecordDifferencesController.PutFhirRecordDifferenceAsync(randomFhirRecordDifference);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            fhirRecordDifferenceServiceMock
               .Verify(service => service.ModifyFhirRecordDifferenceAsync(inputFhirRecordDifference),
                   Times.Once);
