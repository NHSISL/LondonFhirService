    ...
    public partial class FhirRecordDifferencesControllerTests
    {
        ...
        public async Task ShouldReturnCreatedOnPostAsync()
        {
            ...
            fhirRecordDifferenceServiceMock
                .Setup(service => service.AddFhirRecordDifferenceAsync(inputFhirRecordDifference))
                    .ReturnsAsync(addedFhirRecordDifference);

            // when
            ActionResult<FhirRecordDifference> actualActionResult = await fhirRecordDifferencesController.PostFhirRecordDifferenceAsync(randomFhirRecordDifference);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            fhirRecordDifferenceServiceMock
               .Verify(service => service.AddFhirRecordDifferenceAsync(inputFhirRecordDifference),
                   Times.Once);
