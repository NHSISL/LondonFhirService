...
namespace LondonFhirService.Manage.Tests.Unit.Controllers.Metrics
{
    public partial class MetricsControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnDeleteByIdsAsync()
        {
            ...
            metricServiceMock
                .Setup(service => service.RemoveMetricByIdAsync(
                    It.IsAny<Guid>(),
                    It.IsAny<CancellationToken>()))
                        .ReturnsAsync(storageMetric);

            // when
            ActionResult<Metric> actualActionResult = await metricsController.DeleteMetricByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            metricServiceMock
                .Verify(service => service.RemoveMetricByIdAsync(
                    It.IsAny<Guid>(),
                    It.IsAny<CancellationToken>()),
                        Times.Once);
