    ...
    public partial class FhirRecordsControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnDeleteByIdAsync()
        {
            ...
            fhirRecordServiceMock
                .Setup(service => service.RemoveFhirRecordByIdAsync(inputId))
                    .ReturnsAsync(storageFhirRecord);

            // when
            ActionResult<FhirRecord> actualActionResult =
                await fhirRecordsController.DeleteFhirRecordByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            fhirRecordServiceMock
                .Verify(service => service.RemoveFhirRecordByIdAsync(inputId),
                    Times.Once);
