...
export function PendingComparisonList(
    ...
    // Reported above the rows rather than instead of them. React Query keeps the last good data
    // through a failure, so returning early here hid records that were still queued - and the
    // empty state below the table went on referring to "the records above", which were no longer
    // on screen. A failed check means this list may be stale, not that it is wrong.
    const failureNotice = error === null
        ? null
        : (
            <div className="alert alert-warning py-2 mb-3" role="alert">
                This list may be out of date - we could not check what is still waiting to be
        ...
        <>
            ...
            <div className="border rounded mb-3">
                ...
                <p className="text-muted small px-3 mt-1 mb-2">
                    These records have arrived from their providers and are queued for the
                    comparison service. This list refreshes on its own, and each one moves into the
                    table below as soon as it has been compared.
                </p>
                        ...
                        {pendingComparisons.map(pendingComparison => (
                            <tr key={pendingComparison.id}>
                                <td className="text-nowrap">
                                    {pendingComparison.landedAtText}
                                </td>

                                <td className="text-break">
                                    {pendingComparison.sourceNameText}
                                    <PrimarySourceBadge
                                        isPrimarySource={pendingComparison.isPrimarySource} />
                                </td>
