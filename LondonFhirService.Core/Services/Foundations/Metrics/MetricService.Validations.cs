...
namespace LondonFhirService.Core.Services.Foundations.Metrics
{
    ...
    {
        private static void ValidateMetricOnAdd(Metric metric)
        {
            ...
            Validate(
                ...
                (Rule: IsInvalid(metric.Id), Parameter: nameof(Metric.Id)),
                (Rule: IsInvalid(metric.CorrelationId), Parameter: nameof(Metric.CorrelationId)),
                (Rule: IsInvalid(metric.Method), Parameter: nameof(Metric.Method)),
                (Rule: IsInvalid(metric.Name), Parameter: nameof(Metric.Name)),
                (Rule: IsInvalid(metric.Started), Parameter: nameof(Metric.Started)),
                (Rule: IsInvalid(metric.Completed), Parameter: nameof(Metric.Completed)),
                (Rule: IsInvalid(metric.CreatedDate), Parameter: nameof(Metric.CreatedDate)),
                (Rule: IsInvalid(metric.Type), Parameter: nameof(Metric.Type)),
                (Rule: IsInvalid(metric.Status), Parameter: nameof(Metric.Status)),
                (Rule: IsGreaterThan(metric.Method, 255), Parameter: nameof(Metric.Method)),
                (Rule: IsGreaterThan(metric.Name, 255), Parameter: nameof(Metric.Name)),
                (Rule: IsGreaterThan(metric.Target, 255), Parameter: nameof(Metric.Target)),
                (Rule: IsGreaterThan(metric.ErrorCode, 100), Parameter: nameof(Metric.ErrorCode)),
                (Rule: IsGreaterThan(metric.Consumer, 255), Parameter: nameof(Metric.Consumer)),
                (Rule: IsNegative(metric.DurationMs), Parameter: nameof(Metric.DurationMs)),
                (Rule: IsNegative(metric.PayloadBytes), Parameter: nameof(Metric.PayloadBytes)),

                (Rule: IsBefore(
                    firstDate: metric.Completed,
                    secondDate: metric.Started,
                    secondDateName: nameof(Metric.Started)),
                Parameter: nameof(Metric.Completed)),

                (Rule: IsSame(
                    firstId: metric.ParentId,
                    secondId: metric.Id,
                    secondIdName: nameof(Metric.Id)),
                Parameter: nameof(Metric.ParentId)));
        }
        ...
        private static void ValidateMetricId(Guid metricId) =>
            Validate(
                ...
                validations: (Rule: IsInvalid(metricId), Parameter: nameof(Metric.Id)));
        ...
        private static void ValidateMetricIsNotNull(Metric metric)
        {
            if (metric is null)
            {
        ...
        private static void ValidateMetricsIsNotNull(List<Metric> metrics)
        {
            if (metrics is null)
            {
