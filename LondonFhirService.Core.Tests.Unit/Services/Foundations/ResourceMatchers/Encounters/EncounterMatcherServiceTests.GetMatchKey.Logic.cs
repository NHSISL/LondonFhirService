...
namespace LondonFhirService.Core.Tests.Unit.Services.Foundations.ResourceMatchers.Encounters
{
    public partial class EncounterMatcherServiceTests
    {
        ...
        [Fact]
        public async Task ShouldReturnNullOnGetMatchKeyWhenEncounterHasNoIdentifierPropertyAsync()
        {
            // given
            string randomId = GetRandomString();

            JsonElement encounterResource =
                CreateEncounterResourceWithoutIdentifierProperty(id: randomId);
        ...
        [Fact]
        public async Task ShouldReturnNullOnGetMatchKeyWhenEncounterHasNonDdsIdentifierAsync()
        {
            // given
            string randomId = GetRandomString();
            JsonElement encounterResource = CreateNonDdsEncounterResource(id: randomId);
            Dictionary<string, JsonElement> resourceIndex = CreateResourceIndex();
