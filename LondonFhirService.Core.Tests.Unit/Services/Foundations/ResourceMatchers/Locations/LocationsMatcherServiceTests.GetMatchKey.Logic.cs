...
namespace LondonFhirService.Core.Tests.Unit.Services.Foundations.ResourceMatchers.Locations
{
    public partial class LocationMatcherServiceTests
    {
        ...
        [Fact]
        public async Task ShouldReturnNullOnGetMatchKeyWhenLocationHasNoIdentifierPropertyAsync()
        {
            // given
            string randomId = GetRandomString();

            JsonElement locationResource =
                CreateLocationResourceWithoutIdentifierProperty(id: randomId);
        ...
        [Fact]
        public async Task ShouldReturnNullOnGetMatchKeyWhenLocationHasNonOdsSiteCodeIdentifierAsync()
        {
            // given
            string randomId = GetRandomString();
            JsonElement locationResource = CreateNonOdsLocationResource(id: randomId);
            Dictionary<string, JsonElement> resourceIndex = CreateResourceIndex();
