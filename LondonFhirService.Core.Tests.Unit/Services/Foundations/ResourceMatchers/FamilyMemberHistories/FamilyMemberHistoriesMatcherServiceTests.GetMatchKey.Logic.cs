...
namespace LondonFhirService.Core.Tests.Unit.Services.Foundations.ResourceMatchers.FamilyMemberHistories
{
    public partial class FamilyMemberHistoryMatcherServiceTests
    {
        ...
        [Fact]
        public async Task ShouldReturnNullOnGetMatchKeyWhenFamilyMemberHistoryHasNoIdentifierPropertyAsync()
        {
            // given
            string randomId = GetRandomString();

            JsonElement familyMemberHistoryResource =
                CreateFamilyMemberHistoryResourceWithoutIdentifierProperty(id: randomId);
